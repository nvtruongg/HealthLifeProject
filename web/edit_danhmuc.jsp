        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-8">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 mb-8 max-w-4xl mx-auto">
                    <h3 class="text-xl font-bold text-gray-800 mb-4">Chỉnh sửa Danh mục: ${danhMucToEdit.tenDanhMuc}</h3>
                    
                    <form action="admin_danhmuc" method="POST" class="space-y-4">
                        <input type="hidden" name="action" value="update">
                        <!-- Gửi cả ID để biết cập nhật record nào -->
                        <input type="hidden" name="id" value="${danhMucToEdit.id}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="tenDanhMuc" name="tenDanhMuc" required
                                       value="${danhMucToEdit.tenDanhMuc}"
                                       class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm 
                                              focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            
                            <div>
                                <label for="idDanhMucCha" class="block text-sm font-medium text-gray-700">Danh mục cha</label>
                                <select id="idDanhMucCha" name="idDanhMucCha"
                                    ...
                                    <c:forEach var="dm" items="${danhMucList}">
                                        <%-- Logic để 'selected' đúng danh mục cha hiện tại --%>
                                        <option value="${dm.id}" ${dm.id == danhMucToEdit.idDanhMucCha ? 'selected' : ''}>
                                            ${dm.tenDanhMuc}
                                        </option>
                                    </c:forEach>
                                </select>
                        ...
                        <div>
                            ...
                            <textarea id="moTa" name="moTa" rows="4"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm 
                                             focus:outline-none focus:ring-blue-500 focus:border-blue-500">${danhMucToEdit.moTa}</textarea>
                        </div>
                        
                        <div class="text-right space-x-3">
                            <!-- Nút Hủy, quay về trang danh sách -->
                            <a href="admin_danhmuc"
